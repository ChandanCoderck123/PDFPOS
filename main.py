...
def process_purchase_order():
    # pdf_path can be dynamically assigned via uploaded file or fixed path for now
    pdf_path = "C:/Users/Chandan Kumar/OneDrive - Work Store Limited/Desktop/PDFPOS/testpodone.pdf"
    ...
    # Prepare schema string for prompt
...
""",
    ...
    try:
        if hasattr(final_output, 'raw'):
            final_data = json.loads(clean_json(final_output.raw))
        elif hasattr(final_output, 'data'):
            final_data = final_output.data
        else:
            raise ValueError("Unsupported final_output format")

        # Validate using Pydantic schema
        PurchaseOrder(**final_data)

        if hasattr(final_output, 'errors'):
